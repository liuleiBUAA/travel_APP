...
PB = "/home/ubuntu/tools/travel_APP/scripts/la_tmp_pb"
IMG = f"{ROOT}/images"
MAPS = f"{ROOT}/maps"
OUT = "/home/ubuntu/.hermes-bot2/media_cache/sf_verify"
os.makedirs(OUT, exist_ok=True)

# ---- 图库索引（复刻后端 ImageIndex.images_for / first_image）----
manifest = json.load(open(f"{IMG}/美国西部/manifest.json", encoding="utf-8"))
...
def render_city(d):
    # hero：城市同名图没有时，用第一个有图的景点作封面（复刻后端 main.py 逻辑）
    city_hero = first_image(d.get("city","")) or first_image(d["name"])
    if not city_hero:
        for a in d.get("attractions", []):
            fi = first_image(a["name"])
            if fi:
                city_hero = fi; break
    if city_hero:
        h = [f'<div class="hero"><img class="hero-img" src="{city_hero}"><div class="hero-mask"></div><div class="hero-ttl"><div class="hero-name">{esc(d["name"])}</div><div class="hero-sub">{esc(d.get("summary","")[:40])}</div></div></div>']
    else:
    ...
    tbody = []
    if tmap:
        mp = f"{MAPS}/{tmap.split('瑞士/')[-1]}" if "瑞士/" in tmap else f"{MAPS}/{tmap}"
...
targets = {
    "01_city_sf.html": "旧金山城市攻略.json",
    "02_attr_alcatraz.html": "恶魔岛.json",
    "03_attr_ggb.html": "金门大桥.json",
    "04_attr_wharf.html": "渔人码头.json",
    "05_attr_ggp.html": "金门公园.json",
    "06_attr_stanford.html": "斯坦福大学.json",
    "07_attr_halfmoon.html": "半月湾.json",
    "08_attr_santacruz.html": "SantaCruz.json",
}
